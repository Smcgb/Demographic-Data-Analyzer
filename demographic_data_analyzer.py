...
def calculate_demographic_data(print_data=True):
  ...
  higher_education_rich = round((higher_ed_rich / higher_ed * 100), 1)

  lower_education_rich = round((lower_ed_rich / lower_ed * 100), 1)
  ...
  highest_earning_country = (
    df[['native_country', 'salary'
        ]].query('salary == ">50K"').groupby('native_country').count() /
    df[['native_country', 'salary'
        ]].groupby('native_country').count()).sort_values(
          by='salary', ascending=False).index[0]
  ...
  highest_earning_country_percentage = round((
    ...
    df[['native_country', 'salary'
          ...
          by='salary', ascending=False).iloc[0, 0] * 100, 1)

  # Identify the most popular occupation for those who earn >50K in India.
  top_IN_occupation = df[(df['native_country'] == 'India') & (
